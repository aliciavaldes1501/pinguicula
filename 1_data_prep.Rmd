...
ping_17_long <- full_join(
  ping_17 %>% 
    ...
    select(plot,id,x,y,comments,temp,revision,date),
  ping_17 %>% 
    pivot_longer(cols=starts_with("stage"),names_to="revision",
                 names_prefix="stage",values_to="stage") %>%
    select(plot,id,x,y,comments,temp,revision,stage)
  ) %>%
...
date_lastV_17  <- ping_17_long %>%
  ...
  select(id,date_lastV)

ping_17_FFD <- ping_17_long %>%
  select(id,date,stage,V_F) %>%
  filter(!is.na(V_F)) %>%
  ...
  select(id,stage_V,stage_F,date_firstF_F,date_lastV_V) %>%
...
ping_20_peak <- ping_20_peak %>% 
  ...
  select(-(Plot:`Comments on seedcounts`)) %>%
...
ping_17_data <- left_join((ping_17_FFD %>% 
                             select(id,FFD_corr)),
                          (ping_17 %>% 
                             select(plot,id,x,y,peak,comments,temp)))%>%
  left_join(ping_17_peak) %>%
  left_join(ping_17_seedpods %>% select (id,n_seedpods)) %>%
  left_join(ping_17_seeds %>% select(plot,id,n_seeds,comments_seeds)) %>%
  # Assign zero seeds to plants with zero seedpods
  mutate(n_seeds=ifelse(n_seedpods==0,0,n_seeds)) %>%
...
ping_20_data <- left_join((ping_20_FFD %>% 
                             select(id,FFD_corr)),
                          (ping_20 %>% 
                             select(plot,id,x,y,peak,comments,start_treatment,
                                    treatment,temp)))%>%
