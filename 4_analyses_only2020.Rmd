...
ggplot(ping_20_data,aes(x=temp,y=FFD_corr))+
  ...
  my_theme()
ggsave(filename="output/figures/FFD_temp.tiff",device="tiff",
...
# FFD_corr:P1:temp
slp <- function(av_fitness_2020) coef(av_fitness_2020)[11]
b <- car::Boot(av_fitness_2020,slp, R=10000) # note the capital B
b1 <- boot::boot.ci(b,type="bca")
...
b <- car::Boot(av_fitness_2020,slp, R=10000) # note the capital B
b1 <- boot::boot.ci(b,type="bca")
F1_P1_ci <- as.data.frame(b1$bca[1,4:5])
rm(slp, b, b1)
...
slp <- function(av_selection_2020) coef(av_selection_2020)[11]
b <- car::Boot(av_selection_2020,slp, R=10000) # note the capital B
b1 <- boot::boot.ci(b,type="bca")
...
aster_predict_long <- reshape(as.data.frame(aster_predict), 
                              varying = list(vars), direction = "long", 
                              timevar = "varb", times = as.factor(vars), 
                              v.names = "resp")

# add artifice
aster_predict_long$seeds <- as.integer(ifelse(
  aster_predict_long$varb == "n_seeds", 1, 0))

aster_predict.df <- aster_predict_long

# fixed model predictions
aster1.p <- predict(aster1, newdata = aster_predict_long,
                    varvar = varb,idvar = id, root = root,
                    info.tol = 1e-11, se.fit = T)

aster_predict.df$fit <- aster1.p$fit
aster_predict.df$se <- aster1.p$se.fit

aster_predict.df.n_seed <- aster_predict.df %>%
  filter(varb == "n_seeds")

aster_predict.df.n_seed<-aster_predict.df.n_seed%>%
  mutate(ymin=fit-se,ymax=fit+se)

P.labs <- c("Non-pollinated","Pollinated")
names(P.labs) <- c(0,1)
ggplot(aster_predict.df.n_seed)+
...
ggplot(aster_predict.df.n_seed,aes(x=FFD_c,y=fit,group=as.factor(temp_c)))+
...
# Fitness in normal scale
ggplot(aster_predict.df.n_seed,aes(x=temp_c,y=fit,group=as.factor(FFD_c)))+
